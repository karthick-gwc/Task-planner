...
import { Bell, Sun, Moon, X, CheckCheck, Trash2, Loader2, Clock } from 'lucide-react';
...
export function Header({ title }: { title?: string }) {
  ...
  const creatingOverdueTaskIds = useRef(new Set<string>());
  ...
  useEffect(() => {
    ...
    myTasks.forEach((task) => {
      ...
      const alreadyNotified = notifications.some(
        (notification) =>
          notification.taskId === task.id && notification.title === 'Task Overdue'
      );
      ...
      if (
        ...
        active &&
        !alreadyNotified &&
        !creatingOverdueTaskIds.current.has(task.id)
      ) {
        creatingOverdueTaskIds.current.add(task.id);
        dispatch(
          createNotification({
            notification: {
              title: 'Task Overdue',
          ...
          creatingOverdueTaskIds.current.delete(task.id);
        });
      }
    });
  }, [tasks, notifications, currentUser?.id, currentUser?.role, dispatch]);
