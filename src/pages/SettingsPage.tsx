...
const Toggle = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
  <button
    ...
    role="switch"
    className={cn(
      'relative w-11 h-6 rounded-full border-none cursor-pointer transition-colors duration-200 shrink-0 focus-visible:outline-2 focus-visible:outline-brand-500 focus-visible:outline-offset-2',
...
export function SettingsPage() {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col gap-5 pb-8">
      ...
      {/* Heading */}
      <div>
        <h1 className="text-[var(--text)] font-bold text-2xl sm:text-[1.6rem] font-display m-0">Settings</h1>
        <p className="text-[var(--text-muted)] text-sm mt-1.5">Manage your preferences and account</p>
      </div>

      {/* ── Profile ── */}
      <Section icon={<User className="w-4 h-4 text-brand-500" />} title="Profile">
        <div className="flex items-center gap-3.5 mb-5 p-3.5 sm:p-4 rounded-xl bg-[var(--surface-3)]">
          ...
          <div className="min-w-0">
            <p className="text-[var(--text)] font-semibold text-[0.95rem] m-0 truncate">{user?.name}</p>
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <Input label="Email Address" value={user?.email ?? ''} disabled helperText="Email cannot be changed" className="opacity-60" />
        </div>
      </Section>

      {/* ── Appearance ── */}
      <Section icon={<Palette className="w-4 h-4 text-brand-500" />} title="Appearance" delay={0.08}>
        <p className="text-[var(--text-muted)] text-sm mb-3.5">Choose your preferred theme</p>
          ...
          {(['light', 'dark'] as const).map((t) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex items-center gap-3 p-3.5 sm:p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 text-left w-full',
                  active
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[var(--text)] font-semibold text-sm m-0 capitalize">{t} Mode</p>
      ...
      {/* ── Notifications ── */}
      <Section icon={<Bell className="w-4 h-4 text-brand-500" />} title="Notifications" delay={0.16}>
      ...
      {/* ── Role & Permissions ── */}
      <Section icon={<Shield className="w-4 h-4 text-brand-500" />} title="Role & Permissions" delay={0.24}>
      ...
      <div className="flex justify-end pt-1 pb-2">
        <button
          onClick={handleSave}
          className="flex items-center gap-2 px-6 py-2.5 rounded-xl border-none bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold cursor-pointer shadow-[0_3px_12px_rgba(86,85,234,0.3)] transition-colors duration-200 active:scale-[0.97]"
        >
